...
const PRESET_COLORS = [
  { name: 'blue', hex: '#00E5FF', bgClass: 'bg-[#00E5FF]' },
  { name: 'pink', hex: '#FF4081', bgClass: 'bg-[#FF4081]' },
  { name: 'yellow', hex: '#FFD740', bgClass: 'bg-[#FFD740]' },
  { name: 'green', hex: '#00E676', bgClass: 'bg-[#00E676]' },
  { name: 'gray', hex: '#888888', bgClass: 'bg-[#888888]' }
];
...
const Login = ({ onLogin, onRegister, onDeleteProfile, existingUsers }) => {
  ...
  const [regGoal, setRegGoal] = useState("LEVEL UP DAILY");
  ...
  const handleCreateProfile = async () => {
    ...
    const exists = existingUsers.some(u => u.name.toUpperCase() === regName.trim().toUpperCase());
    if (exists) {
    ...
    await onRegister({
      ...
      avatar: regAvatar,
      color: regColor,
      defaultGoal: regGoal.trim().toUpperCase() || "LEVEL UP DAILY",
      pin: regPin
    });
    setIsLoading(false);
    ...
    setRegName("");
    setRegPin("");
    setRegGoal("LEVEL UP DAILY");
    setShowRegister(false);
  };

  const handleDeleteConfirm = () => {
    if (deletingUser) {
      if (deletePin === deletingUser.pin) {
        onDeleteProfile(deletingUser.uid);
    ...
    return (
      <div className={`min-h-screen ${THEME.bg} flex items-center justify-center p-6 font-mono`}>
        <div className="w-full max-w-md">
          <Card className="animate-in fade-in zoom-in-95 duration-300 border-red-500">
            <div className="text-center mb-8">
              <h2 className="text-lg font-bold text-red-500 uppercase tracking-widest mb-2">Delete Profile</h2>
              <p className="text-xs text-[#888] uppercase mb-6">Enter PIN for {deletingUser.name} to confirm permanent deletion</p>
              <Avatar emoji={deletingUser.avatar} size="lg" className="mx-auto border-red-500" />
            </div>
            <input 
                type="password" 
                inputMode="numeric"
                ...
                className="w-full bg-[#111111] py-4 text-center text-white text-xl tracking-[0.5em] font-mono border border-red-500/50 focus:border-red-500 outline-none mb-6 placeholder-[#444]"
            />
            <div className="flex gap-4">
                <Button onClick={() => { setDeletingUser(null); setDeletePin(""); }} variant="secondary" className="flex-1">CANCEL</Button>
                <Button onClick={handleDeleteConfirm} className="flex-[2] bg-red-600 hover:bg-red-700 text-white border-transparent">
                    DELETE PROFILE
                </Button>
            </div>
          </Card>
      ...
      <div className={`min-h-screen ${THEME.bg} flex items-center justify-center p-6 font-mono`}>
        <div className="w-full max-w-md my-8">
          <Card className="animate-in fade-in zoom-in-95 duration-300">
            <h2 className="text-lg font-bold text-white uppercase tracking-widest text-center mb-8">Create New Profile</h2>
            <div className="space-y-6">
              <div>
                <label className={THEME.fontSmallCaps}>PROFILE NAME</label>
                <input 
                  type="text" 
                  maxLength={12}
                  value={regName}
                  ...
                  className="w-full bg-[#111111] p-3 text-white border border-[#333333] focus:border-white outline-none font-mono text-sm uppercase placeholder-[#444]"
                />
              </div>

              <div>
                <label className={THEME.fontSmallCaps}>4-DIGIT PIN (SECURITY)</label>
                <input 
                  type="password" 
                  inputMode="numeric"
                  maxLength={4}
                  ...
                  placeholder="PIN"
                  className="w-full bg-[#111111] p-3 text-white border border-[#333333] focus:border-white outline-none font-mono text-sm uppercase tracking-widest placeholder-[#444]"
                />
              </div>

              <div>
                <label className={THEME.fontSmallCaps}>DEFAULT GOAL</label>
                <input 
                  type="text" 
                  maxLength={25}
                  value={regGoal}
                  onChange={e => setRegGoal(e.target.value)}
                  placeholder="E.G. LEVEL UP DAILY"
                  className="w-full bg-[#111111] p-3 text-white border border-[#333333] focus:border-white outline-none font-mono text-sm uppercase placeholder-[#444]"
                />
              </div>

              <div>
                <label className={THEME.fontSmallCaps}>SELECT EMOJI AVATAR</label>
                <div className="grid grid-cols-6 gap-2 mt-2">
                  {PRESET_AVATARS.map(emoji => (
                    <button 
                      ...
                      className={`text-2xl p-2 bg-[#222] border transition-all ${regAvatar === emoji ? 'border-white bg-[#333]' : 'border-[#333] hover:border-[#555]'}`}
                    >
                      {emoji}
                    </button>
                  ))}
              ...
              <div>
                <label className={THEME.fontSmallCaps}>SELECT THEME COLOR</label>
                <div className="flex gap-3 mt-2">
                  {PRESET_COLORS.map(colorObj => (
                    <button 
                      key={colorObj.name}
                      type="button"
                      onClick={() => setRegColor(colorObj.name)}
                      className={`w-8 h-8 rounded-full border-2 transition-all ${colorObj.bgClass} ${regColor === colorObj.name ? 'border-white scale-110' : 'border-transparent'}`}
                    />
                  ))}
                </div>
              </div>
              ...
              <div className="flex gap-4 pt-4">
                  <Button onClick={() => setShowRegister(false)} variant="secondary" className="flex-1">CANCEL</Button>
                  <Button onClick={handleCreateProfile} className="flex-[2]" disabled={isLoading}>
        ...
        {!selected ? (
            <div className="grid grid-cols-2 gap-4">
                {existingUsers.map(u => (
                    <div 
                        key={u.uid}
                        className={`group ${THEME.card} p-8 border ${THEME.border} hover:border-white transition-all flex flex-col items-center justify-between relative`}
                    >
                        <button 
                          onClick={(e) => { e.stopPropagation(); setDeletingUser(u); }}
                          className="absolute top-2 right-2 text-[#444] hover:text-red-500 transition-colors p-1"
                ...
                <button 
                    ...
                    className={`group ${THEME.card} p-8 border ${THEME.border} border-dashed hover:border-white transition-all flex flex-col items-center justify-center gap-6`}
                >
                    <div className="w-16 h-16 rounded-full border-2 border-dashed border-[#444] group-hover:border-white flex items-center justify-center transition-all">
                        <Plus size={24} className="text-[#666] group-hover:text-white transition-colors" />
                    </div>
                    <span className="text-sm font-bold text-[#666] group-hover:text-white uppercase tracking-widest">New Profile</span>
                </button>
            </div>
        ) : (
            <Card className="animate-in fade-in zoom-in-95 duration-300">
                ...
                />
                <div className="flex gap-4">
                    <Button onClick={() => { setSelected(null); setPin(""); }} variant="secondary" className="flex-1">BACK</Button>
...
const Dashboard = ({ users, currentUser, setView, onSelectPartner }) => { 
  const otherUsers = users.filter(u => u.uid !== currentUser.uid);
  const partner = users.find(u => u.uid === currentUser.partnerId) || otherUsers[0];
    ...
    <div className="space-y-6 pb-20 max-w-4xl mx-auto animate-in fade-in slide-in-from-bottom-4">
      ...
      <Card className="flex items-center gap-6 bg-[#1A1A1A]">
          {partner ? (
              ...
              <div className="flex-1">
                  <div className="flex justify-between items-center mb-2">
                      <div className="flex items-center gap-3">
                          <span className="text-sm font-bold text-white uppercase tracking-widest">{partner.name}</span>
                          <span className="text-[10px] text-[#666] font-mono">LVL {calculateLevel(partner.points || 0)}</span>
                          {otherUsers.length > 1 && (
                              <select 
                                value={partner.uid}
                                onChange={(e) => onSelectPartner(e.target.value)}
                                className="bg-[#222] text-[10px] font-mono uppercase tracking-wider border border-[#333] px-2 py-1 text-[#888] hover:text-white outline-none focus:border-white cursor-pointer transition-colors"
                              >
                                {otherUsers.map(u => (
                                  <option key={u.uid} value={u.uid}>Partner: {u.name}</option>
                                ))}
                              </select>
                          )}
                      </div>
            ...
            <div className="w-full text-center py-4 text-[#666] text-xs uppercase tracking-widest flex flex-col items-center gap-1 font-mono">
              <div>Awaiting partner connection...</div>
              <div className="text-[10px] text-[#444]">Create another profile to link accounts!</div>
            </div>
          )}
      </Card>
...
export default function App() {
  ...
  const handleRegister = async (newUserData) => {
    try {
      ...
      const userRef = doc(db, 'artifacts', appId, 'public', 'data', COLLECTION_NAMES.USERS, newUid);
      await setDoc(userRef, fullUserData);
      showToast("Profile Created");
      await handleLogin(fullUserData);
    } catch (e) {
  ...
  const handleDeleteProfile = async (uid) => {
    try {
      await deleteDoc(doc(db, 'artifacts', appId, 'public', 'data', COLLECTION_NAMES.USERS, uid));
  ...
  const handleSelectPartner = async (partnerId) => {
    ...
    try {
      const userRef = doc(db, 'artifacts', appId, 'public', 'data', COLLECTION_NAMES.USERS, currentUserData.uid);
      await updateDoc(userRef, { partnerId });
      setCurrentUserData(prev => ({ ...prev, partnerId }));
      showToast("Partner Connected");
    } catch (e) {
      console.error("Select Partner Error:", e);
      showToast("Error connecting partner.");
    }
  };
